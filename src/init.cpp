...
void Shutdown()
{
    ...
    RenameThread("DiscountCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: DiscountCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: DiscountCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDiscountCoinamount=<n> " + _("Keep N DiscountCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DiscountCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DiscountCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DiscountCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DiscountCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DiscountCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DiscountCoin to complete") << "\n";
    ...
    nAnonymizeDiscountCoinAmount = GetArg("-anonymizeDiscountCoinamount", 0);
    if(nAnonymizeDiscountCoinAmount > 999999) nAnonymizeDiscountCoinAmount = 999999;
    if(nAnonymizeDiscountCoinAmount < 2) nAnonymizeDiscountCoinAmount = 2;
    ...
    LogPrintf("Anonymize DiscountCoin Amount %d\n", nAnonymizeDiscountCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DSC+1000 == (.1DSC+100)*10
       10DSC+10000 == (1DSC+1000)*10
    */
