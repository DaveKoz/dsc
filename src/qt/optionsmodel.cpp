...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDiscountCoinAmount"))
        settings.setValue("nAnonymizeDiscountCoinAmount", 1000);
    nAnonymizeDiscountCoinAmount = settings.value("nAnonymizeDiscountCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDiscountCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDiscountCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDiscountCoinAmount:
            return QVariant(nAnonymizeDiscountCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDiscountCoinAmount:
            nAnonymizeDiscountCoinAmount = value.toInt();
            settings.setValue("nAnonymizeDiscountCoinAmount", nAnonymizeDiscountCoinAmount);
            emit AnonymizeDiscountCoinAmountChanged(nAnonymizeDiscountCoinAmount);
            break;
        default:
            break;
        }
