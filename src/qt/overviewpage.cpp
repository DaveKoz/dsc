...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDiscountCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDiscountCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDiscountCoinAmount = strAnonymizeDiscountCoinAmount.remove(strAnonymizeDiscountCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDiscountCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDiscountCoinAmount*COIN) nMaxToAnonymize = nAnonymizeDiscountCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDiscountCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDiscountCoinAmount));
        strAnonymizeDiscountCoinAmount = strAnonymizeDiscountCoinAmount.remove(strAnonymizeDiscountCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDiscountCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDiscountCoinAmount == 0){
